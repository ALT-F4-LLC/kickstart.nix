...
build-darwin system="x86_64": (clean-template "darwin")
    ...
    rsync -a --chmod=ugo=rwX --progress $OUTPUT/* /tmp/kickstart.nix/darwin
...
build-language template profile="default" system="x86_64": (clean-template template)
    ...
    OUTPUT=$(echo $DERIVATION | jq -r ".[0].outputs.out")
    rsync -a --chmod=ugo=rwX --progress $OUTPUT/* /tmp/kickstart.nix/{{ template }}
...
build-nixos-desktop system="x86_64" desktop="gnome": (clean-template "nixos-desktop")
    ...
    OUTPUT=$(echo $DERIVATION | jq -r ".[0].outputs.out")
    rsync -a --chmod=ugo=rwX --progress $OUTPUT/* /tmp/kickstart.nix/nixos-desktop
    nix build --json --no-link --print-build-logs \
...
build-nixos-minimal system="x86_64": (clean-template "nixos-minimal")
    ...
    OUTPUT=$(echo $DERIVATION | jq -r ".[0].outputs.out")
    rsync -a --chmod=ugo=rwX --progress $OUTPUT/* /tmp/kickstart.nix/nixos-minimal
    nix build --json --no-link --print-build-logs \
